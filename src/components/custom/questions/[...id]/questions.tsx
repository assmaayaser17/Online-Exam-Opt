...
// import results from'@/components/custom/results/Result'
...
import React, { useContext, useEffect, useState } from "react";
import { QuestionsContext } from '@/components/providers/context/QuestionsContext';
import Result from "@/components/custom/Result/Result"; // Import your result component
...
const Questions: React.FC<QuestionsProps> = ({ onClose }) => {
  ...
  const [questions, setQuestions] = useState<Question[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [showResultPopup, setShowResultPopup] = useState(false); // State for result popup
  const { handleAnswerSelect, selectedAnswers } = useContext(QuestionsContext);
      ...
      {showResultPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-[600px]">
            <Result onClose={() => setShowResultPopup(false)} />
          </div>
        </div>
      )}
    </>
